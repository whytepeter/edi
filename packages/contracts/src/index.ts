...
export { skinGeometrySchema, skinGeometry, handPaths, mapSkinPoint, desktopPetSize, type SkinGeometry } from './skin-geometry';
...
export const modelIdSchema = z.string().trim().min(3).max(160).regex(/^[a-zA-Z0-9_.:/-]+$/);
export const agentStateSchema = z.object({
  configured: z.boolean(), model: z.string(),
  status: z.enum(['idle', 'running', 'done', 'stopped', 'error']),
  text: z.string().max(32000), error: z.string(),
});
...
export const commandSchema = z.discriminatedUnion('type', [
  z.object({ type: z.literal('request-listening'), mode: z.enum(['conversation', 'push-to-talk']).default('conversation') }).strict(),
  z.object({ type: z.literal('release-listening'), cancelled: z.boolean() }).strict(),
  z.object({ type: z.literal('character-action'), action: z.enum(['conversation', 'content', 'stop', 'sleep', 'quit', 'dismiss']) }).strict(),
  z.object({ type: z.literal('character-menu') }).strict(),
  z.object({ type: z.literal('configure-agent'), apiKey: z.string().trim().min(10).max(512), model: modelIdSchema }).strict(),
  ...
  z.object({ type: z.literal('pet-drag'), phase: z.enum(['start', 'move', 'end', 'cancel']),
    pointerId: z.number().int().nonnegative(), point: screenPointSchema }).strict(),
]);
