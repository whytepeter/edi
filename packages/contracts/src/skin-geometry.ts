...
const bounds = z.object({ x: z.number().finite(), y: z.number().finite(), width: z.number().positive().finite(), height: z.number().positive().finite() }).strict();
export const skinGeometrySchema = z.object({
  version: z.literal(1),
  viewBox: bounds,
  ...
  const inside = (p: { x: number; y: number }) => p.x >= box.x && p.y >= box.y && p.x <= box.x + box.width && p.y <= box.y + box.height;
  for (const [name, value] of Object.entries(geometry.anchors)) {
    if (!inside(value)) context.addIssue({ code: 'custom', message: 'Anchor outside viewBox', path: ['anchors', name] });
  }
  const painted = geometry.paintedBounds;
  if (!inside(painted) || !inside({ x: painted.x + painted.width, y: painted.y + painted.height }))
    context.addIssue({ code: 'custom', message: 'Painted bounds outside viewBox', path: ['paintedBounds'] });
});
...
const base = {
  ...
  anchors: { workspace: { x: 31, y: 40 }, leftHand: { x: 30, y: 99 }, rightHand: { x: 132, y: 112 } },
};
export const skinGeometry = {
  cloud: skinGeometrySchema.parse({ ...base,
    paintedBounds: { x: 8, y: 38, width: 146, height: 108 },
    bodyPath: 'M31 91C31 59 51 40 79 40C109 40 131 62 131 91C131 121 109 143 80 143C50 143 31 121 31 91Z',
    decorationPath: '',
  }),
  sprout: skinGeometrySchema.parse({ ...base,
    ...
    decorationPath: 'M80 39C58 33 57 14 59 12C76 13 86 22 80 39ZM81 39C81 21 96 17 106 20C103 34 96 40 81 39Z',
  }),
...
export function mapSkinPoint(geometry: SkinGeometry, point: { x: number; y: number }, viewport: { x: number; y: number; width: number; height: number }) {
  if (![viewport.x, viewport.y, viewport.width, viewport.height, point.x, point.y].every(Number.isFinite) || viewport.width <= 0 || viewport.height <= 0)
