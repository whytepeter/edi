    ...
    >
      <header className="card-header">
        ...
        <div className="header-actions">
          <button
            className={`icon-button ${settings.pinned ? 'is-on' : ''}`}
            aria-label={settings.pinned ? 'Unpin card' : 'Pin card'}
      ...
      {menu && (
        <>
          <button className="menu-dismiss" aria-label="Close menu" onClick={() => setMenu(false)} />
          <div className="options">
            <button onClick={() => navigate('agent')}>
              Talk to Edi <span>↗</span>
            </button>
        ...
        {view === 'content' && (
          <section className="intro-view">
            <div className="hello-visual">
              <div className="halo" />
              <Pet skin={settings.skin} />
              <span className="hello-note">hey, you.</span>
            </div>
            <h1>
              A little space.
              <br />
            ...
            <p>
              Edi’s explanations, media, and questions
              <br />
              will appear here as you talk.
            </p>
            <button className="preview-button" onClick={() => navigate('agent')}>
              Talk to Edi <span>↗</span>
            </button>
            <span className="preview-label">TEMPORARY CONNECTION TEST</span>
          </section>
        )}
        {view === 'avatars' && (
          <section>
            <div className="eyebrow">APPEARANCE</div>
            <h1>Pick your little someone.</h1>
            <p className="subtitle">Same Edi. A different face.</p>
            <div className="avatar-grid">
              {skins.map(skin => (
                <button
                  key={skin.id}
                  className={`avatar-choice ${preview === skin.id ? 'selected' : ''}`}
                  aria-label={`${skin.name} avatar option`}
                  aria-pressed={preview === skin.id}
                  onClick={() => setPreview(skin.id)}
                >
                  <Pet skin={skin.id} />
                  <span>{skin.name}</span>
                  <small>{settings.skin === skin.id ? 'Your Edi' : 'Try a new look'}</small>
                  {preview === skin.id && (
                    <i>
                      <Icon name="check" />
                    </i>
            ...
            <button
              ...
              disabled={preview === settings.skin}
              onClick={() => void send({ type: 'apply-skin', skin: preview })}
            >
              {preview === settings.skin
                ? 'This is your Edi'
                : `Use ${skins.find(s => s.id === preview)!.name}`}
            </button>
            <p className="fine-print">Your voice and conversations stay the same.</p>
          </section>
        )}
        {view === 'extensions' && (
          <section>
            <div className="eyebrow">EXTENSIONS</div>
            <h1>A few more possibilities.</h1>
            <input
              className="search"
            ...
            <div className="filters">
              {['All', 'Skills', 'Connectors', 'MCP', 'Skins'].map(kind => (
                <button
                  key={kind}
            ...
            <div className="extension-list">
              {items.map(item => (
                <article key={item.name}>
                  <span className="extension-symbol">{item.symbol}</span>
                  <div>
                    <h2>{item.name}</h2>
                    <p>{item.detail}</p>
                    <small>{item.available ? 'Included' : 'Coming later · not connected'}</small>
                  </div>
                  {item.available && (
                    <button
                      className="icon-button"
              ...
              {items.length === 0 && <p className="fine-print">No matches. Try another search.</p>}
            </div>
            <p className="fine-print">Catalog preview. External installs are not enabled yet.</p>
          </section>
        )}
        {view === 'activity' && (
          <section>
            ...
            <div className="quiet">
              ...
              <p>
                When Edi starts helping, actions
                <br />
