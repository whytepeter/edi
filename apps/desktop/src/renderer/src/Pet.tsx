...
export function Pet({ skin, className = '' }: { skin: SkinId; className?: string }) {
  ...
  return <svg className={`pet-art ${className}`} data-skin={sprout ? 'sprout' : 'cloud'} viewBox={`${viewBox.x} ${viewBox.y} ${viewBox.width} ${viewBox.height}`} preserveAspectRatio="xMidYMid meet" fill="none" aria-label={`${sprout ? 'Sprout' : 'Cloud'} avatar`} role="img">
    ...
    <g className="pet-body" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
      ...
      {(['left', 'right'] as const).map(side => {
        ...
        return <path key={side} data-part={`${side}-hand`} transform={`translate(${anchor.x} ${anchor.y})`} d={handPaths[side]} fill={sprout ? '#f0f5ec' : '#f5f8ff'}/>;
      })}
