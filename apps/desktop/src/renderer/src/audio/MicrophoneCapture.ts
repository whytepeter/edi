...
async function acquire(signal: AbortSignal, dependencies: CaptureDependencies): Promise<MediaStream> {
  ...
  return new Promise((resolve, reject) => {
    ...
    void Promise.resolve().then(() => dependencies.getUserMedia({
      audio: { channelCount: 1, echoCancellation: true, noiseSuppression: true }, video: false,
    })).then(stream => {
      if (settled || signal.aborted) { stopTracks(stream); fail(abortError()); return; }
...
export async function openMicrophone(
  ...
  if (signal.aborted) { stopTracks(stream); throw abortError(); }
  let recorder: MediaRecorder;
  try {
    if (!stream.getAudioTracks().some(track => track.readyState === 'live')) throw new Error('No live microphone track');
  ...
  const result = new Promise<Blob | null>((resolve, reject) => { resolveResult = resolve; rejectResult = reject; });
  ...
  const cleanup = () => {
    clearTimeout(durationTimer); clearTimeout(startTimer); clearTimeout(flushTimer);
  ...
  const settle = (error?: Error, discard = false) => {
    ...
    if (error) rejectResult(error); else resolveResult(blob);
  };
  const cancel = () => settle(undefined, true);
  const deviceLost = () => { if (!finishing) settle(new Error('Microphone disconnected')); };
  const durationTimer = setTimeout(() => settle(new Error('Recording exceeded 60 seconds')), 60_000);
  ...
  recorder.onstart = () => { started = true; clearTimeout(startTimer); resolveReady(); };
  recorder.ondataavailable = event => {
    if (settled || !event.data.size) return;
    bytes += event.data.size;
    if (bytes > 8 * 1024 * 1024 || chunks.length >= 512) { settle(new Error('Recording size limit exceeded')); return; }
    chunks.push(event.data);
  };
  recorder.onerror = () => settle(new Error('Microphone recording failed'));
  recorder.onstop = () => finishing ? settle() : settle(new Error('Microphone stopped unexpectedly'));
  try {
    if (signal.aborted) cancel(); else recorder.start(250);
  } catch { settle(new Error('Microphone recording could not start')); }
  await ready;
  return {
    result,
    cancel,
