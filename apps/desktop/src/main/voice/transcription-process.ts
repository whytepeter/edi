...
export interface TranscriptionRuntime { executable: string; model: string; vadModel: string }

/** Fixed-format boundary: 16 kHz mono signed PCM16 little-endian, at most 61 s. */
export function pcmWave(pcm: Uint8Array): Buffer {
  if (!(pcm instanceof Uint8Array) || !pcm.byteLength || pcm.byteLength % 2 || pcm.byteLength > 16000 * 2 * 61) {
  ...
  wav.write('data', 36); wav.writeUInt32LE(pcm.byteLength, 40); wav.set(pcm, 44);
  return wav;
}

/** A single offline utterance. Paths belong to native configuration, never renderer input. */
export async function transcribePcm(runtime: TranscriptionRuntime, pcm: Uint8Array,
  signal: AbortSignal, timeoutMs = 30_000): Promise<string> {
  signal.throwIfAborted();
  const wav = pcmWave(pcm);
  return new Promise((resolve, reject) => {
    const child = spawn(runtime.executable, ['-m', runtime.model, '--vad', '-vm', runtime.vadModel,
      '-f', '-', '-l', 'en', '-t', '4', '-ng', '-np', '-nt', '-otxt', '-of', '-'], {
    ...
    child.stdout.on('data', (data: Buffer) => {
      size += data.length;
      if (size > 32000) { fail('Transcription output limit exceeded'); return; }
      output.push(data);
    });
    child.on('close', code => {
      ...
      wav.fill(0);
      if (failure || code !== 0) { reject(failure ?? new Error('Transcription failed')); return; }
      const text = Buffer.concat(output).toString('utf8').trim();
      if (text.length > 8000) { reject(new Error('Transcript too long')); return; }
