...
function publish() {
  for (const win of [workspace, pet])
    if (!win.isDestroyed()) win.webContents.send('edi:settings', settings);
}
...
function createWindows() {
  ...
  workspace.on('blur', () => {
    if (!settings.pinned) workspace.hide();
  });
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  void app.whenReady().then(async () => {
    try {
      settings = settingsSchema.parse(JSON.parse(await readFile(settingsPath(), 'utf8')));
    } catch {
    ...
    agent = new AgentService(state => {
      if (!workspace.isDestroyed()) workspace.webContents.send('edi:agent', state);
    });
    await agent.load();
    createWindows();
    const trusted = (event: Electron.IpcMainInvokeEvent) => {
      if (
        ![workspace.webContents, pet.webContents].includes(event.sender) ||
    ...
    ipcMain.handle('edi:command', async (event, raw: unknown) => {
      ...
      const command = commandSchema.parse(raw);
      if (
        command.type === 'request-listening' ||
        command.type === 'release-listening' ||
        command.type === 'character-menu'
      ) {
      ...
      if (command.type === 'pet-drag') {
        if (event.sender !== pet.webContents) throw new Error('Pet command only');
        await petDrag.handle(command);
        return;
      }
      if (command.type === 'pet-hit-test') {
        if (event.sender !== pet.webContents) throw new Error('Pet command only');
        if (!petDrag.active) pet.setIgnoreMouseEvents(!command.interactive, { forward: true });
        return;
      }
      if (command.type === 'show-workspace') {
        placement.show();
        return;
      }
      if (event.sender !== workspace.webContents) throw new Error('Workspace command only');
      if (command.type === 'configure-agent') {
        await agent.configure(command.apiKey, command.model);
        return;
      }
      if (command.type === 'disconnect-agent') {
        await agent.disconnect();
        return;
      }
      if (command.type === 'ask-agent') {
        agent.ask(command.prompt);
        return;
      }
      if (command.type === 'stop-agent') {
        agent.stop();
        return;
      }
      ...
      if (command.type === 'set-expanded') {
        placement.place({
          x: 0,
          y: 0,
          width: command.expanded ? 740 : 408,
          height: command.expanded ? 650 : 480,
        });
        return;
      }
