...
export class AgentService {
  ...
  private path() { return join(app.getPath('userData'), 'openrouter.enc'); }
  ...
  async load() {
    ...
    } catch { /* Missing or locked credentials require setup again. */ }
  }
  async configure(apiKey: string, model: string) {
    if (this.configuring || this.worker) throw new Error('Stop the response before changing the connection.');
  ...
  ask(prompt: string) {
    ...
    worker.on('message', message => {
      ...
      if (message.type === 'text' && typeof message.text === 'string') {
        if (this.state.text.length + message.text.length > 32000) {
          finish('error', 'Response reached the display limit. Ask for a shorter answer.'); return;
        }
        this.state = { ...this.state, text: this.state.text + message.text };
        this.emit();
      } else if (message.type === 'done') {
        finish(this.state.text ? 'done' : 'error', this.state.text ? '' : 'No text was returned. Try a text-capable model.');
      } else finish('error', 'OpenRouter could not complete this response. Check your key, model ID, credits, and connection.');
    });
    ...
    this.timeout = setTimeout(() => finish('error', 'The response timed out. You can try again.'), 120000);
  }
  stop() {
    clearTimeout(this.timeout);
