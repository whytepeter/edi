...
try {
  ...
  await expect.poll(() => app.windows().filter(page => page.url().includes('surface=')).length).toBe(2);
  ...
  const visible = surface => app.evaluate(({ BrowserWindow }, surface) =>
    BrowserWindow.getAllWindows().find(win => win.webContents.getURL().includes(`surface=${surface}`))?.isVisible(), surface);
  const action = label => app.evaluate(({ Menu }, label) => {
    const item = Menu.getApplicationMenu().items[0].submenu.items.find(item => item.label === label);
  ...
  await expect.poll(() => app.windows().some(page => page.url().includes('surface=voice-status'))).toBe(true);
  ...
  expect(await visible('workspace')).toBe(false);
  expect(await app.evaluate(({ globalShortcut }) => globalShortcut.isRegistered('CommandOrControl+Shift+E'))).toBe(true);
  await action('Listen');
  await expect.poll(() => visible('pet')).toBe(true);
  expect(await visible('workspace')).toBe(false);
  const rejected = await workspace.evaluate(async () => {
    try { await window.edi.command({ type: 'request-listening' }); return false; }
    catch { return true; }
  });
  expect(rejected).toBe(true);
  await action('Stop');
  await expect.poll(() => app.windows().filter(page => page.url().includes('voice-status')).length).toBe(0);
  await pet.evaluate(() => window.edi.command({ type: 'character-menu' }));
  await expect.poll(() => app.windows().some(page => page.url().includes('surface=character-menu'))).toBe(true);
  ...
  expect(await menu.evaluate(() => document.querySelector('.character-menu').getBoundingClientRect().bottom <= innerHeight - 8)).toBe(true);
  await menu.screenshot({ path: join(profile, 'character-menu.png') });
  await menu.keyboard.press('Escape').catch(error => { if (!menu.isClosed()) throw error; });
  await expect.poll(() => app.windows().some(page => page.url().includes('surface=character-menu'))).toBe(false);
  ...
  await expect.poll(() => app.windows().some(page => page.url().includes('surface=voice-status'))).toBe(true);
  await pet.mouse.up();
  await expect.poll(() => app.windows().some(page => page.url().includes('surface=voice-status'))).toBe(false);
  expect(await visible('workspace')).toBe(false);
  console.log(`Character interactions passed. Screenshot: ${join(profile, 'voice-unavailable.png')}`);
} finally {
  await app?.close();
}
