...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    try {
      ...
      turn.finish();
      const endedImmediately = destination.stream.getTracks().every(track => track.readyState === 'ended');
      ...
      const secondTurn = await globalThis.openTestCapture(cancelled.signal, {
        ...deps, getUserMedia: async () => second.stream,
      });
      cancelled.abort();
      ...
      return { source: 'synthetic oscillator, no microphone', bytes: blob.size,
        mimeType: blob.type, decodedSeconds: decoded.duration, endedImmediately,
        discarded, cancelReleased,
