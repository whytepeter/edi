...
try {
  ...
  const report = await page.evaluate(async bytes => {
    ...
    try {
      ...
      const fullPlayback = { ...player.snapshot(), elapsedMs: performance.now() - start,
        firstGraphSignalMs: firstSignalMs, backpressureCount };
      ...
      const interruption = { stopCallMs, graphSilenceObservedMs, staleResult, stayedSilent,
        ...player.snapshot() };
      await player.dispose();
      return { schemaVersion: 1, timestamp: new Date().toISOString(), muted: true,
        source: 'Pocket run 2 greeting; synthetic 80 ms chunks / 20 ms arrival spacing',
        audibleLatencyMeasured: false, decodedSampleRate: audio.sampleRate,
        audioSeconds: audio.duration, baseLatency: context.baseLatency,
        fullPlayback, interruption, disposedState: context.state };
    } finally {
      await player.dispose();
      analyser.disconnect();
      mute.disconnect();
