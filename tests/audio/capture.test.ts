...
import { openMicrophone, type CaptureDependencies } from '../../apps/desktop/src/renderer/src/audio/MicrophoneCapture';
...
function fixture() {
  ...
  const recorder = {
    ...
    onstart: null as null | (() => void), onstop: null as null | (() => void),
    onerror: null as null | (() => void), ondataavailable: null as null | ((event: { data: Blob }) => void),
    start() { this.state = 'recording'; queueMicrotask(() => this.onstart?.()); },
    stop() { this.state = 'inactive'; queueMicrotask(() => {
      this.ondataavailable?.({ data: new Blob(['final']) }); this.onstop?.();
    }); },
  };
  const deps: CaptureDependencies = { getUserMedia: async () => stream,
...
test('permission denial and unsupported recording fail without retained tracks', async () => {
  ...
  denied.deps.getUserMedia = async () => { throw new DOMException('Denied', 'NotAllowedError'); };
  await assert.rejects(openMicrophone(new AbortController().signal, denied.deps), { name: 'NotAllowedError' });
  const unsupported = fixture();
  unsupported.deps.createRecorder = () => { throw new Error('Unsupported'); };
...
test('size overflow and device disconnection discard and release capture', async () => {
  for (const mode of ['size', 'device']) {
    const f = fixture();
    const turn = await openMicrophone(new AbortController().signal, f.deps);
    if (mode === 'size') f.recorder.ondataavailable?.({ data: new Blob([new Uint8Array(8 * 1024 * 1024 + 1)]) });
