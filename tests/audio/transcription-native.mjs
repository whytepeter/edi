...
const runtime = {
  executable: join(runtimeRoot, 'whisper.cpp-927cfce34f31707e17f2bff35c349632fb9e2c3a/build/bin/whisper-cli'),
  model: join(runtimeRoot, 'ggml-base.en.bin'), vadModel: join(runtimeRoot, 'ggml-silero-v6.2.0.bin'),
};
let compiled = '';
...
try {
  ...
  const audio = await page.evaluate(async bytes => {
    ...
    try {
      ...
      return { pcm: [...pcm], recordedBytes: blob.size,
        tracksEnded: stream.stream.getTracks().every(track => track.readyState === 'ended') };
    } finally {
      controller.abort(); source.disconnect();
  ...
  const transcript = await transcribePcm(runtime, new Uint8Array(audio.pcm), new AbortController().signal);
  ...
  try {
    await assert.rejects(transcribePcm(runtime, new Uint8Array(audio.pcm), controller.signal), /cancelled/);
  } finally { clearTimeout(cancelTimer); }
  console.log(JSON.stringify({ transcript, transcriptionMs, recordedBytes: audio.recordedBytes,
    tracksEnded: audio.tracksEnded, silenceRejected: silence === '', runningCancellationPassed: true,
    physicalMicrophone: false, cloudCalls: 0 }, null, 2));
